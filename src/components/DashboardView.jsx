...
function Dashboard2TabPane({ csvHeaders, csvRows }) {
...
function DashboardView({ fileName, csvHeaders, csvRows, onBackHome }) {
  ...
  const [dashboardNumbers, setDashboardNumbers] = useState([2])

  const dashboard1 = useDashboard1(csvHeaders, csvRows)

  const tabs = [
    { id: 'dashboard1', label: 'Dashboard 1', number: 1 },
    ...dashboardNumbers.map((number, index) => ({
      id: `dashboard${number}`,
  ...
  const canAddDashboard = tabs.length < MAX_DASHBOARDS
  ...
  const handleAddDashboard = () => {
    ...
    setDashboardNumbers((previous) => {
      const nextNumber = Math.max(...previous, 1) + 1
      const next = [...previous, nextNumber]
      setActiveTab(`dashboard${nextNumber}`)
      return next
    })
  }

  const handleRemoveDashboard = (dashboardNumber) => {
    if (dashboardNumber < 3) {
      return
    }

    setDashboardNumbers((previous) => {
      const next = previous.filter((number) => number !== dashboardNumber)
      if (activeTab === `dashboard${dashboardNumber}`) {
        const fallback = next[next.length - 1] ?? 2
  ...
  const activeTabNumber = Number(activeTab.replace('dashboard', ''))

  const currentPageTitle = activeTab === 'dashboard1'
    ? 'Dashboard 1 Overview'
    : `Dashboard ${Number.isFinite(activeTabNumber) ? activeTabNumber : 2} Overview`
    ...
    <main className="app-page">
              ...
              {tab.number >= 3 && (
                <button
                  ...
                  onClick={(event) => {
                    ...
                    handleRemoveDashboard(tab.number)
                  }}
                >
                  ×
                </button>
      ...
      <section className="page-content">
        <article className="panel panel--secondary">
          <section hidden={activeTab !== 'dashboard1'}>
            <Dashboard1
          ...
          {dashboardNumbers.map((dashboardNumber) => {
            const tabId = `dashboard${dashboardNumber}`
            
            if (dashboardNumber === 2) {
              return (
                <section key={tabId} hidden={activeTab !== tabId}>
                  <Dashboard2TabPane csvHeaders={csvHeaders} csvRows={csvRows} />
                </section>
              )
            }
            ...
            return (
              <section key={tabId} hidden={activeTab !== tabId}>
                <DynamicDashboardView csvHeaders={csvHeaders} csvRows={csvRows} />
              </section>
            )
          })}
        </article>
